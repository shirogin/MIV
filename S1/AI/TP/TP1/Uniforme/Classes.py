...
class Uniform(Informe):
    ...
    def draw(self):
        if (not self.finished[0]) and len(self.ouvert) > 0:
            self.ouvert.sort(key=lambda obj: obj.cout)
            current = self.ouvert.pop(0)
            if current.Type == 0:
                current.Type = 5
            for son in current.children:
                if(son == self.objective):
                    self.finished = [True, son]
                elif(son.Type == 0) and (son not in self.ferme) and (son not in self.ouvert) and (son.cout > (son.Cout(current)+current.cout)):
                    son.cout = son.Cout(current) + current.cout
                    son.parent = current
                    self.ouvert.append(son)
                print(son.Type, son.cout, son.Cout(current) + current.cout)
            self.ferme.append(current)
            drawTable(self.noeds)
            return self.finished
        else:
